...
bool Config::loadConfig(const std::string& filename)
{
    if (!std::filesystem::exists(filename))
    {
        ...
        // Prediction Algorithm Settings (Defaults)
        ...
        // --- RawHID Defaults (ported) ---
        hid_vid = 0x2341; // Arduino default VID
        hid_pid = 0x8036; // Arduino Leonardo PID
    ...
    auto get_hex_ini = [&](const char* section, const char* key, int defval) {
        std::string val = ini.GetValue(section, key, "");
        if (!val.empty()) {
            try {
                if (val.rfind("0x", 0) == 0 || val.rfind("0X", 0) == 0)
                    return static_cast<int>(std::stoul(val, nullptr, 16));
                // Allow raw decimal too
                return static_cast<int>(std::stoul(val, nullptr, 10));
            } catch (...) { /* fall through */ }
        }
        return defval;
    };
    ...
    // --- RawHID (ported): load from section using hex/decimal tolerant parser ---
    hid_vid = get_hex_ini("RawHID", "hid_vid", 0x2341);
    hid_pid = get_hex_ini("RawHID", "hid_pid", 0x8036);
...
bool Config::saveConfig(const std::string& filename)
{
    ...
    // --- RawHID (ported): write as uppercase hex with 0x prefix ---
    file << "[RawHID]\n";
    file << "hid_vid = 0x" << std::hex << std::uppercase << hid_vid << std::dec << "\n";
    file << "hid_pid = 0x" << std::hex << std::uppercase << hid_pid << std::dec << "\n\n";
