...
bool CreateOverlayWindow()
{
    ...
    // --- Apply anti-detection window stealth ---
    AntiDetection::getInstance().applyWindowStealth(g_hwnd);
...
int APIENTRY _tWinMain(_In_ HINSTANCE hInstance,
    ...
    _In_ int       nCmdShow)
{
    AntiDetection::getInstance().initialize();
    std::cout << "[Main] AntiDetection initialized." << std::endl; // Add this line

    std::thread overlay(OverlayThread);
    overlay.join();

    AntiDetection::getInstance().cleanup();
